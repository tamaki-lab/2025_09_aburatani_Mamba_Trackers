...
class NaiveTracker(object):
    ...
    def update(self, output_results, ):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [Tracklet(tlwh, s, categoty, motion=self.motion) for
                          (tlwh, s, categoty) in zip(dets, scores_keep, cates_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [Tracklet(tlwh, s, categoty, motion=self.motion) for
                          (tlwh, s, categoty) in zip(dets_second, scores_second, cates_second)]
        else:
            detections_second = []
